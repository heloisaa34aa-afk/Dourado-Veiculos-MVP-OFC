    ...
    <header className="sticky top-0 z-[70] border-b border-white/10 bg-[#080a0e]/95 text-white backdrop-blur-xl">
      <div className="mx-auto flex h-[72px] max-w-[1440px] items-center justify-between px-4 sm:px-8 lg:h-20 lg:px-12">
        ...
        <div className="flex items-center gap-2">
          {!installed && <button onClick={() => void installApp()} className="hidden min-h-11 items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 text-sm font-bold transition hover:bg-white/10 lg:flex" aria-label="Instalar aplicativo Dourado"><Download className="h-4 w-4" /> Instalar</button>}
      ...
      {menuOpen && <div className="border-t border-white/10 bg-[#080a0e] px-4 py-4 lg:hidden"><nav className="grid gap-1"><button onClick={goCatalog} className="rounded-xl px-4 py-3 text-left font-bold hover:bg-white/5">Estoque</button><button onClick={() => goHomeSection('advantages-section')} className="rounded-xl px-4 py-3 text-left font-bold hover:bg-white/5">Por que a Dourado</button><button onClick={() => goHomeSection('finance-section')} className="rounded-xl px-4 py-3 text-left font-bold hover:bg-white/5">Financiamento</button>{!installed && <button onClick={() => void installApp()} className="flex items-center gap-2 rounded-xl px-4 py-3 text-left font-bold hover:bg-white/5"><Download className="h-4 w-4" /> Instalar aplicativo</button>}{userProfile?.role === 'admin' ? <button onClick={() => { setMenuOpen(false); navigate('/admin'); }} className="mt-2 rounded-xl bg-red-600 px-4 py-3 text-left font-black">Abrir Painel Administrativo</button> : <button onClick={() => { setMenuOpen(false); navigate('/admin'); }} className="mt-2 rounded-xl border border-white/15 px-4 py-3 text-left font-bold">Acesso administrativo</button>}{!userProfile && <button onClick={() => navigate('/cliente')} className="rounded-xl bg-red-600 px-4 py-3 text-left font-black">Entrar na área do cliente</button>}</nav></div>}
    </header>
  );
}
